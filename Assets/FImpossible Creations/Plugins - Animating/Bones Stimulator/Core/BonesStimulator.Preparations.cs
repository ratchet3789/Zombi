...
namespace FIMSpace.BonesStimulation
{
    public partial class BonesStimulator
    {
        ...
        private void Motion_MotionInfluence()
        {
            ...
            if( InfluenceAxes != Vector3.one )
            {
                influenceOffset = ( CompensationTransform.position - previousPosition );

                Vector3 localOffset = CompensationTransform.InverseTransformDirection( influenceOffset );

                localOffset = Vector3.Scale( localOffset, new Vector3(1f - InfluenceAxes.x, 1f - InfluenceAxes.y, 1f - InfluenceAxes.z ));
                influenceOffset = CompensationTransform.TransformDirection( localOffset );
            }
            else
            {
